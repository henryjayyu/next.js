...
- `failure-monitor.json`: a multi-alert `ci-pipelines alert` that triggers
  ...
  rather than staying open. The alert message links the specific failed
  ...
  No-data notifications are disabled on purpose: Datadog treats zero
  matching events as NO DATA for event-based monitors, and zero failures
  is the healthy state here, so enabling them would notify on every quiet
  window.
- `skip-rate-monitor.json`: a formula-based `ci-pipelines alert` computing
  `skipped runs / all runs` per workflow over a trailing 1 day window,
  warning (no critical threshold) above 0.5. A high skip rate is usually
  benign (`retry-tests`, `retry-deploy-tests`, and
  `Allow issue participants to reopen issues` skip by design when there is
  nothing to do), but it can also mean a condition broke and the workflow
  silently stopped doing work. To exclude a by-design skipper, add
  `-@ci.pipeline.name:"retry-tests"` (etc.) to both variable searches. The
  monitor renotifies once a week while it stays in warning, and it also
  notifies and renotifies on no data: its queries match every run, so a
  full day without any pipeline events means the GitHub integration itself
  stopped reporting, which is the one outage the failure monitor cannot
  detect.
...
documentation. A few things are not documented and must be checked in the
CI Pipelines explorer or on the first alert:

1. Pipeline names: run
   `ci_level:pipeline @ci.pipeline.name:"Lock Threads"` and confirm events
   exist and that the names match the ones inlined in the JSON queries.
2. Skipped runs: check whether fully skipped runs (for example recent
   `Allow issue participants to reopen issues` executions) appear with a
   distinguishable status such as `@ci.status:skipped`. The explorer docs
   only list Success, Failure, and Canceled. If skipped runs are not
   distinguishable, delete `skip-rate-monitor.json`; a polling reporter
   that reads conclusions from the GitHub API would be needed instead.
3. `startup_failure` runs: a run whose workflow file is invalid starts
   ...
   class.
4. Message variables: on the first failure alert, confirm the rendered
...
5. If importing `skip-rate-monitor.json` fails over the `group_by` inside
   its formula variables, build that monitor in the UI instead (New
   Monitor -> CI/CD & Tests -> Pipelines, formula `query1 / query2`
   grouped by pipeline name) and export it back over this file.
