import { resolveUpgrade, checkCodemod, readUpgradeRouters } from './resolve'
...
function dependencies(): UpgradeDependencies {
  return {
    now: () => now,
    readRouters: jest.fn(async () => app.routers),
    nodeVersion: '22.0.0',
...
describe('upgrade resolution', () => {
  it('rejects a monorepo root without routers and recognizes its configless app', async () => {
    ...
    try {
      ...
      expect(deps.readApp).toHaveBeenCalledWith(selected, config, ['app'])
      ...
      expect(
        await resolveUpgrade({ directory: selected }, disabled)
      ).toMatchObject({
        status: 'disabled',
        reason: expect.stringContaining('agenticAutoUpgrade'),
      })
      expect(disabled.readApp).not.toHaveBeenCalled()
      expect(disabled.readSecurity).not.toHaveBeenCalled()
    } finally {
  ...
  it('returns disabled before installed packages, network or tool resolution', async () => {
    const deps = { ...dependencies(), loadConfig: jest.fn(async () => ({})) }
    expect(await resolveUpgrade({ directory: '/app' }, deps)).toMatchObject({
      status: 'disabled',
    })
    expect(deps.readApp).not.toHaveBeenCalled()
    expect(deps.readSecurity).not.toHaveBeenCalled()
  ...
  it('pins the selected target and codemod in a noninteractive command', async () => {
    const deps = dependencies()
    const result = await resolveUpgrade({ directory: '/app' }, deps)
    expect(result).toMatchObject({
      status: 'ready',
