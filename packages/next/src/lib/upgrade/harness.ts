...
export async function findHarnesses(
...
export function launchHarness(
  ...
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(
      harness,
      ['--bg', '--model', UPGRADE_MODELS.claude.cli, prompt],
      {
        cwd: directory,
    ...
    const printNativeLine = (line: string) => {
      console.log(
        process.platform === 'win32'
          ? line
          : line.replace(
              /^(\s*)claude (?=(?:agents|attach|logs|stop|kill|rm|respawn)\b)/,
              '$1command claude '
            )
      )
    }
    output?.on('line', (line) => {
      ...
      if (
        session &&
        /^\s*claude (?:agents|attach|logs|stop|kill|rm|respawn)\b/.test(plain)
      ) {
        nativeReceipt.push(line)
        return
      }
    ...
    child.once(
      ...
      (code: number | null, signal: NodeJS.Signals | null) => {
        ...
        if (code === 0 && !signal && session) {
          ...
          for (const line of nativeReceipt) printNativeLine(line)
        }
...
export async function handoffUpgrade(
  ...
  console.log(
    `\n  ${bold('Next.js security update available!')}${
      versions
        ? ` ${dim(versions.current)} → ${cyan(bold(versions.target))}`
        : ''
    }\n`
  )
