...
const workflow = '01-app/02-guides/upgrading/security-upgrade.md'
const migrationWorkflow = '01-app/02-guides/upgrading/security-migration.md'
...
describe('upgrade resources', () => {
  ...
  async function prepareFixture() {
    ...
    const workflowPath = join(docs, workflow)
    await writeFile(
      workflowPath,
      'Check repository preflight, then read security-migration.md.'
    )
    const migrationWorkflowPath = join(docs, migrationWorkflow)
    await writeFile(
  ...
  it('retains complete relevant docs, original paths and context after package replacement', async () => {
    ...
    expect(packet.prompt).not.toContain(
      'This is a --experimental-agent-dry-run'
    )
    expect(packet.context.app.manifestHash).toBe('baseline')
  ...
  it('fails honestly and removes only its partial packet when required guides cannot be fetched', async () => {
    ...
    expect(await readFile(join(options.docsRoot, workflow), 'utf8')).toContain(
      'preflight'
    )
  })
})
