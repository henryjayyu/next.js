...
program
  ...
  .action(async (directory, options, command) => {
    ...
    await mod.spawnNextUpgrade(directory, {
      ...options,
      revision:
        // A supplied revision must match the security policy's selected target.
        // Commander's default is not a user request and may point to a different
        // major, so pass undefined to let the policy choose without that constraint.
