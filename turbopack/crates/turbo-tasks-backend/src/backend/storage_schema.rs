...
impl TaskStorage {
    ...
    /// The tasks holding a cell dependency on this one, as `(edge kind, dependent task)` pairs.
    ///
    /// Used to check the invariant that makes ignoring cell dependencies safe (see
    /// [`TaskStorage::gc_maybe_collectible`]): every dependent of a collected task must itself be
    /// collected in the same pass.
    #[cfg(debug_assertions)]
    pub fn gc_cell_dependents(&self) -> GcRootHolders {
