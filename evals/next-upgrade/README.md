...
This is an independent `@vercel/agent-eval` project for `next upgrade --agent --dry-run`.
...
The original `codex` and `claude` experiments use the shared defaults in
`packages/next/src/lib/upgrade/models.ts`, with overrides through
...
launcher and evaluates its completed work, including `--dry-run` behavior.
...
F1 uses a Python pseudo-terminal to run `next upgrade --agent --dry-run`. The actual
...
so the explicit `--dry-run` request must narrow delivery. Sandbox wrappers record
